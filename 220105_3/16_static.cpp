...
//Car.cpp
int Car::cnt = 0;
//1. static 정적 멤버 데이터 변수는 반드시 클래스 외부에 선언해야 한다.
...
//정리
//1. 정적 멤버 데이터
// - 모든 객체가 공유하는 멤버 데이터, 객체를 생성하지 않ㄷ아도 메모리 존재한다.
// - 전역 변수와 달리 접근 지정자를 사용할 수 있고, 클래스 안에 포함된기 때문에 관련되 데이터를 묶어서 관리할 수 있다.
//2. 정적 멤버 함수
// - 객체 없이 호출할 수 있는 멤버 함수이다.
// - 객체 없이 호출 되므로 일반 함수와 유사하지만, 접근 지정자를 사용할 수 있고, 특정 타입과 관련된 함수를 묶어서 관리할 수 있습니다.