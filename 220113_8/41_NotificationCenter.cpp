...
public: 
    ...
    //핵심
    // map<std::string, HANDLER> h; //이렇게 만들면 1개밖에 등록 불가  
    map<std::string, std::vector<HANDLER>> notifiMap;

public:
    void AddObserver(const std::string& name, HANDLER h){ notifiMap[name].push_back(h); }
